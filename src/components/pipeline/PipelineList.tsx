    ...
    <div className="w-full h-[620px] p-3 flex flex-col gap-3">
      <div className="w-full min-h-[520px] mt-10">
        {data.map((item, index) => (
          <div
            key={index}
            className="w-full flex flex-row gap-3 px-3 py-2 border-[1px] border-gray1"
          >
            <div className="mt-2">
              {item.status === "ing" ? (
            ...
            <div className="w-[700px]">
              ...
              <p className="truncate ... w-full font-thin text-gray8 text-[0.8rem]">
                {item.commitInfo + item.commitInfo + item.commitInfo}
              </p>
            </div>
            <div>
              <Button className="mt-3" size="small" disabled>
                {item.branch}
              </Button>
            </div>
            ...
            <div className="flex flex-row gap-2 mt-2">
              <p>
                <PlayCircleFilled
                  className={`${
                    item.status === "ing"
                      ? "text-gray5"
                      : "text-gray10 hover:text-black cursor-pointer"
                  }`}
                  style={{ fontSize: 30 }}
                />
              </p>
              <p>
                <PauseCircleFilled
                  className="text-gray10 hover:text-black cursor-pointer"
                  style={{ fontSize: 30 }}
                />
              </p>
            </div>
