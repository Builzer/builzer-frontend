...
export default function ProjectMenuList({...props}) {
    ...
    return <div className='mt-2 h-[170px] border-b-2 border-gray1 overflow-y-auto pb-2'>
        {projects.map((project: projectList, index: number) => (
            <div key={index} className={`p-1 cursor-pointer hover:bg-gray1 ${selectedProject.projectId === project.projectSpecId ? 'bg-gray2 hover:bg-gray2' : ''}`} onClick={() => handleProjectId(project.projectSpecId, project.projectName)}>{project.projectName}</div>
        ))}
    </div>
}