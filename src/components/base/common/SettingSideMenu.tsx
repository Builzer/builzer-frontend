...
export default function SettingSideMenu({ ...props }) {
  ...
  return (
    <div className="border-[1px] border-gray1 rounded-lg px-2 py-5 flex flex-col gap-10 shadow-lg">
      {menuList.map((item: string) => (
        <div
          className={`${
            menu === item ? "font-bold" : "font-thin"
          } cursor-pointer`}
          key={item}
      ...
      {isButton ? (
        <Button className="bg-black" type="primary" onClick={handleStartDeploy}>
          재배포하기
        </Button>
      ) : (
        <></>
