...
export default function SelectProjectTemplate() {
  ...
  const setProjectBuildInfo =
    useSetRecoilState<projectBuildInfo>(projectBuildState);
  const [projectDefaultInfo, setProjectDefaultInfo] =
    useRecoilState<projectDefaultInfo>(projectDefaultInfoState);
    ...
    <div className="w-full h-full p-2">
      ...
      <div>
        ...
        <div className="flex flex-row gap-3 mt-2">
          <div
            className={`w-1/2 rounded-md relative cursor-pointer ${
              plan === "Lite" ? "border-2 border-black" : "shadow-lg"
            }`}
            onClick={() => handlePlan(data[0].plan.planName)}
          >
            <img className="m-auto mt-2" src={PlanBg} alt="플랜 배경" />
            <p className="absolute font-bold text-3xl top-8 left-[46%] text-white">
              L
            </p>
          ...
          <div
            className={`w-1/2 rounded-md relative cursor-pointer ${
              plan === "Pro" ? "border-2 border-black" : "shadow-lg"
            }`}
            onClick={() => handlePlan(data[1].plan.planName)}
          >
            <img className="m-auto mt-2" src={PlanBg} alt="플랜 배경" />
            <p className="absolute font-bold text-3xl top-8 left-[46%] text-white">
              P
          ...
          {plan === "Lite" ? (
            <div className="flex flex-row gap-2">
              {data[0].setting.map((item, index) => (
                <div
                  key={index}
                  className={`w-[200px] h-[133px] p-2 rounded-md cursor-pointer ${
                    projectId === item.projectId
                      ? "border-2 border-black"
                      : "shadow-lg"
                  }`}
                  onClick={() => handleRecentSetting(item)}
                >
      ...
      <Button
        ...
        type="primary"
        onClick={() => handleGoProjectSetting()}
      >
        프로젝트 세팅 설정
      </Button>
    </div>
