...
export default function ProjectSelect({ ...props }) {
  ...
  const handleSearch = (
    ...
    if (data) {
      ...
      //플랜 필터 선택 시
      if (plan !== "") {
        tmpProjectList = tmpProjectList.filter(
          (item) => item.projectPlan === plan
        );
      }
      ...
      <div className="mt-12">
        ...
        <div className=" h-[500px] bg-white mt-2 rounded-md overflow-y-auto p-2">
          {projectList.map((project, index) => (
            <div
              key={index}
              className={`cursor-pointer px-2 py-2 flex flex-row justify-between hover:bg-gray1 ${
                projectId === project.projectId ? "bg-gray2 hover:bg-gray2" : ""
              }`}
